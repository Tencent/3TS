...
class PreceInfo {
 public:
  inline PreceInfo(std::shared_ptr<Txn> from_txn, std::shared_ptr<Txn> to_txn, const PreceType type,
      ...
      : from_txn_id_(from_txn->txn_id()),
        from_txn_(std::move(from_txn)),
        to_txn_(std::move(to_txn)),
        type_(type),
        row_id_(row_id),
        from_ver_id_(from_ver_id),
        to_ver_id_(to_ver_id) {}
  ...
  friend std::ostream& operator<<(std::ostream& os, const PreceInfo prece) {
    return os << 'T' << prece.from_txn_id() << "--" << prece.type_ << "(row=" << prece.row_id_ << ")->T"
              << prece.to_txn_id();
  }
...
class Path {
 public:
  ...
  bool IsCycle() const { return preces_.size() >= 2 && preces_.front().from_txn_id() == preces_.back().to_txn_id(); }
...
template <typename Txn>
Path<Txn>::Path(std::vector<PreceInfo<Txn>>&& preces)
    : preces_(
#if CYCLE_ORDER
          std::move(preces))
#else
...
template <typename Txn>
Path<Txn>::Path(const PreceInfo<Txn>& prece)
    : preces_{prece} {}

template <typename Txn>
Path<Txn>::Path(PreceInfo<Txn>&& prece)
    : preces_{std::move(prece)} {}
...
Path<Txn>& Path<Txn>::operator+=(const Path<Txn>& p) {
  ...
  if (current_back.row_id() == append_front.row_id()) {
    ...
    if (current_back.from_txn_id() != append_front.to_txn_id() &&
      ...
      preces_.emplace_back(current_back.from_txn(), append_front.to_txn(), new_prece_type, current_back.row_id(),
                           new_from_ver_id, new_to_ver_id);
...
[[maybe_unused]] static AnomalyType IdentifyAnomalyMultiple(const std::vector<PreceInfo<Txn>>& preces) {
  if (std::any_of(preces.begin(), preces.end(),
                  [](const PreceInfo<Txn>& prece) { return prece.type() == PreceType::WW; })) {
