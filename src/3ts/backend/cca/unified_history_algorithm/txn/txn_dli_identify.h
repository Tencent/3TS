...
class TxnManager<
    ...
    typename std::enable_if_t<
        ALG == UniAlgs::UNI_DLI_IDENTIFY_CYCLE ||
        ALG == UniAlgs::UNI_DLI_IDENTIFY_CHAIN>> : public std::enable_shared_from_this<TxnManager<ALG, Data>> {
 public:
  ...
  void AddToTxn(TxnManager& to_txn, const uint64_t row_id, const uint64_t from_ver_id, const uint64_t to_ver_id,
                const PreceType type) {
    ...
    if (const auto& real_type = RealPreceType_(type); txn_id_ != to_txn_id && real_type.has_value()) {
      ...
      std::shared_ptr<PreceInfo<Txn>> prece = std::make_shared<PreceInfo<Txn>>(
          this->shared_from_this(), to_txn.shared_from_this(), *real_type, row_id, from_ver_id, to_ver_id);
