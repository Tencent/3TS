...
class AlgManager<ALG, Data, typename std::enable_if_t<ALG == UniAlgs::UNI_DLI_IDENTIFY_CYCLE>> {
 public:
  ...
  void Start(const uint64_t txn_id) { return std::make_unique<Txn>(txn_id); }
 ...
 private:
  ...
  std::vector<std::vector<Path<Txn>>> InitPathMatrix_(const std::vector<std::shared_ptr<Txn>>& cc_txns) const {
