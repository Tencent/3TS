...
class RowManager<ALG, Data, typename std::enable_if_t<ALG == UniAlgs::UNI_DLI_IDENTIFY_CYCLE ||
 ...
 public:
  ...
  RowManager(const uint64_t row_id, Data init_data)
      : row_id_(row_id),
        cur_ver_id_(0),
        latest_version_(std::make_shared<Ver>(std::weak_ptr<Txn>(), std::move(init_data), 0)) {}
  ...
  bool Prewrite(Data data, Txn& txn) {
    ...
    const auto pre_version =
        std::exchange(latest_version_, std::make_shared<Ver>(txn.shared_from_this(), std::move(data), to_ver_id));
 ...
 private:
  void build_prece_from_w_txn_(Ver& version, Txn& to_txn, const uint64_t to_ver_id, const PreceType type) const {
    if (const std::shared_ptr<Txn> from_txn = version.w_txn()) {
      build_prece(*from_txn, to_txn, version.ver_id(), to_ver_id, type);
    }
  }

  void build_preces_from_r_txns_(Ver& version, Txn& to_txn, const uint64_t to_ver_id, const PreceType type) const {
    version.foreach_r_txn([&to_txn, to_ver_id, from_ver_id = version.ver_id(), this, type ](Txn & from_txn) {
                                                                                             build_prece(
                                                                                                 from_txn, to_txn,
                                                                                                 from_ver_id, to_ver_id,
                                                                                                 type);
                                                                                           });
  }

  void build_prece(Txn& from_txn, Txn& to_txn, const uint64_t from_ver_id, const uint64_t to_ver_id,
                   const PreceType type) const {
