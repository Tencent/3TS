...
class VerManager<ALG, Data, typename std::enable_if_t<ALG == UniAlgs::UNI_DLI_IDENTIFY_SSI>> {
 public:
  ...
  bool IsWrittenBy(const uint64_t txn_id) const { return w_txn_ && w_txn_->txn_id() == txn_id; }
  ...
  std::vector<std::shared_ptr<Txn>> r_txns_;  // use shared_ptr instead of weak_ptr or readonly transactions will be
                                              // released
};
  ...
  bool Prewrite(Data data, Txn& txn) {
    ...
    {  // logical not contains in SSI ---- build RW precedence
      for (const auto& r_txn : latest_version.r_txns_) {
        ...
        const bool is_concurrent = r_txn_state == Txn::State::ACTIVE || r_txn_state == Txn::State::PREPARING ||
                                   (r_txn_state == Txn::State::COMMITTED && r_txn->commit_ts() > txn.start_ts());
