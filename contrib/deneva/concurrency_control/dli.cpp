...
#include "txn.h"
        ...
        CC_ALG == DLI_MVCC
void dli_merge_set(Dli::RWSet& r_tc, Dli::RWSet& r_tl) {
    ...
    if (rc == RCOK) {
        for (auto& i : wset) {
            if (i.first->manager->w_trans != ts &&
                    !i.first->manager->w_trans.compare_exchange_weak(expect, ts)) {
                rc = Abort;
                break;
            }
        }
...
#if CC_ALG != DLI_DTA && CC_ALG != DLI_DTA2 && CC_ALG != DLI_DTA3
        txn->dli_txn = TSNode<DliValidatedTxn>::push_front(dli->validated_txns_, std::move(rset), std::move(wset), ts);
#else
        txn->dli_txn = TSNode<DliValidatedTxn>::push_front(dli->validated_txns_, std::move(rset), std::move(wset), ts, lower, upper);
...
void Dli::finish_trans(RC rc, TxnManager* txn) {
    ...
    if (rc != RCOK && txn->dli_txn != nullptr) {
        txn->dli_txn->is_abort_ = true;
    } else if (rc == RCOK) {
#if CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || CC_ALG == DLI_DTA3
